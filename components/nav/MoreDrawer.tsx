  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="flex flex-1 flex-col gap-1 p-2">
          {MOBILE_MORE_ITEMS.map(({ href, label, Icon }) => (
            <Link
              ...
              href={href}
              onClick={onClose}
              className="flex h-10 items-center gap-3 px-2 text-body-s font-medium text-ink hover:bg-paper-2 transition-colors"
        ...
        <form action={signOutAction} className="border-t border-paper-3 p-2">
          <button
            type="submit"
            className="flex w-full items-center gap-2 px-2 py-2 text-left text-body-s text-ink hover:bg-paper-2"
            style={{ borderRadius: "4px" }}
          >
